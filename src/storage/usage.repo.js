Майя Артеменко, [06.01.2026 23:10]
src/storage/usage.repo.js
import { query } from "./db.js";

function todayISO() {
  const d = new Date();
  // YYYY-MM-DD в UTC достаточно для лимитов (можно позже сделать по таймзоне)
  return d.toISOString().slice(0, 10);
}

export async function getDailyCount(telegramId, day = todayISO()) {
  const id = String(telegramId);
  const res = await query(
    `select count from usage_daily where telegram_id = $1 and day = $2`,
    [id, day]
  );
  return res.rows[0]?.count ?? 0;
}

export async function incrementDailyCount(telegramId, day = todayISO()) {
  const id = String(telegramId);
  const res = await query(
    `insert into usage_daily (telegram_id, day, count)
     values ($1, $2, 1)
...
Майя Артеменко, [06.01.2026 23:11]
